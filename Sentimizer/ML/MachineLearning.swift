...
//        let task = URLSession.shared.dataTask(with: request) { [self] (data, response, error) in
        let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
            guard let data = data, let actualPath = actualPath else { return }
            do {
    ...
    static func feedforward(ip: [Double]) -> [Double] {
        let defaults = UserDefaults.standard
        do {
            let permanentURL = defaults.string(forKey: K.modelURL)!
            let model = try TestModel(contentsOf: URL(string: permanentURL)!)
            
            guard let mlMultiArrayInput = try? MLMultiArray(shape:[1, NSNumber(value: ip.count)], dataType:MLMultiArrayDataType.double) else { return [] }
